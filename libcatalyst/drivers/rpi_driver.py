...
class RaspberryPiDriver(DriverInterface):
    ...
    def exchange_spi(self, cs_name, write_data, write_bits, read_bits):
        ...
        for i in range(write_bits):
            bit_pos = write_bits - 1 - i
            GPIO.output(MOSI, (write_data >> bit_pos) & 0x1)
            GPIO.output(SCLK, GPIO.HIGH)
            GPIO.output(SCLK, GPIO.LOW)
        
        read_data = 0
        # Read the specified number of bits
        for i in range(read_bits):
            GPIO.output(SCLK, GPIO.HIGH)
            bit = GPIO.input(MISO)
        ...
        write_hex_width = (write_bits + 3) // 4  # Calculate the necessary width of the hexadecimal output for write data
        write_data_format = f"0{write_hex_width}x"  # Format string for write data hexadecimal width
        
        read_hex_width = (read_bits + 3) // 4  # Calculate the necessary width of the hexadecimal output for read data
        read_data_format = f"0{read_hex_width}x"  # Format string for read data hexadecimal width
        
        print(f"SPI Exchange {cs_name} (GPIO{CS}): Write: 0x{format(write_data, write_data_format)}, Read: 0x{format(read_data, read_data_format)}")
